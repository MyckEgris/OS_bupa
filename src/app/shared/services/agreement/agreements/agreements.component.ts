...
import { ConstansAgreements } from 'src/app/shared/services/agreement/constants/constants-agreements';
...
export class AgreementsComponent implements AgreementsModel {
  ...
  */
   ok() {
    ...
    var nPolicyId = 0;
    if(user.policy_id === undefined) {
        this.activeModal.close(true);
    } else
    {
      nPolicyId = Number(user.policy_id);
      const policyLogDto = {
        logTypeId: 86,
        policyId: nPolicyId,
        description: ConstansAgreements.AGREEMENTSOKTEXT,
        memberId: 0,
        userId:user.sub
      };
      this.activeModal.close(true);
      // this.policyService.SavePolicyLog(policyLogDto).subscribe(
      //   p => {
      //     this.activeModal.close(true);
      //   }, e => {
      //       console.error(e);
  ...
  async showAtDeclineAgreementForUserCanBeLocked() {
    ...
    if (user.policy_id === undefined) {
      nPolicyId = 0;
      this.showAtDeclineAgreementForUserCanBeLocked2();
    } else {
      this.showAtDeclineAgreementForUserCanBeLocked2();
    }
  }

  async showAtDeclineAgreementForUserCanBeLocked2() {
    debugger
  ...
  confirmDeclineAgreement() {
    ...
    //guardar respuesta en el log de la poliza
  }
}
