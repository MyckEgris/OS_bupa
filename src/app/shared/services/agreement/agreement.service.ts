...
export class AgreementService {
   ...
   * @param lang Language
   */
  async getAgreementsByRole(roles: Array<string>, lang: string, insuranceBusiness: string) {
    return await this._http.get(`${this._config.apiHost}/${this.AGREEMENTS_BY_ROLE}?roles=${roles.join(',')}&lang=${lang}&insuranceBusiness=${insuranceBusiness}`)
      .toPromise()
  ...
  addAgreementRefusedModel(agreement: Agreement, agreementByUserDto: AgreementUserVersionsDto,
    ...
    if (agreementByUserDto) {
      return {
        ...
        messageToCheck: agreement.messageToCheck 
      };
    } else {
      return {
        agreementId: agreement.agreementId,
   ...
   * @param lang Language
   */
  async getAgreementsToAccept(userId: string, roles: Array<any>, userName: string, lang: string, insuranceBusiness: string) {
    const agreements: any = await this.getAgreementsByRole(roles, lang, insuranceBusiness);
   ...
   * @param lang Language
   */
  async showAgreements(userId: string, roles: Array<any>, userName: string, lang: string, insuranceBusiness: string) {
    const agreements: any = await this.getAgreementsByRole(roles, lang,insuranceBusiness);
    const agreementsByUser: any = await this.getUserAgreementById(userId);
    const refusedAgreements = await this.getAgreementsWithoutAccept(agreements, agreementsByUser, userId, userName);
  ...
  async startAgreements() {
    ...
    if (!user.is_anonymous) {
      const agreements = await this.getAgreementsToAccept(user.sub, user.roles, user.name,
        user.language ? user.language : this.DEFAULT_LANGUAGE,user.bupa_insurance);
  ...
  async showAgreementsDialog(agreement, htmlContent, lastAgreement: boolean) {
    ...
    dialog.messageToCheck = agreement.messageToCheck;
   ...
   * @param title Title
   * @param messagetocheck messageToCheck
   */
  showCustomAgreementsDialog(htmlContent, title: string, declineText?: string, messagetocheck?: string) {
    const result = this.modalService.open(AgreementsComponent, { size: 'lg', centered: true, backdrop: 'static', keyboard: false });
    const dialog = result.componentInstance;
    dialog.title = title;
    dialog.content = htmlContent;
    dialog.acceptText = 'APP.BUTTON.AGREE_BTN';
    dialog.declineText = declineText ? declineText : 'APP.BUTTON.DECLINE_BTN';
    dialog.messageToCheck = messagetocheck;
    return result.result; 
  }

  /**
   * Open modal information popup with dynamically component
   * @param htmlContent Content in Html format
   * @param title Title
   */
   showCustomInformationDialog(htmlContent, title: string, closeText?: string) {
    const result = this.modalService.open(InformationComponent, { size: 'lg', centered: true, backdrop: 'static', keyboard: false });
    const dialog = result.componentInstance;
    dialog.title = title;
    dialog.content = htmlContent;
    dialog.acceptText = closeText ? closeText : 'APP.CERRAR';
    return result.result;
  }
