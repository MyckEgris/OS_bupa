...
@Component({
  selector: 'app-moral-petitioner',
  templateUrl: './moral-petitioner.component.html'
})
export class MoralPetitionerComponent implements OnInit, OnDestroy {
  ...
  setUpForm() {
    ...
    (this.wizard.enrollmentForm.get(this.configStep.type) as FormGroup)
      .addControl(
        this.configStep.sections.find(s => s.id === this.POLICYAPP_INFO_MORAL_PETITIONER).name,
        this.policyEnrollmentWizardService.buildSection(this.currentStep, this.POLICYAPP_INFO_MORAL_PETITIONER)
      );
  }
