...
import { PolicyAppViewHelper } from '../policy-application-view/policy-application-view.helper';
import { WindowResizeService } from 'src/app/shared/services/window-resize/windowresize.service';
import { Subscription } from 'rxjs';

@Component({
  selector: 'app-policy-enrollment-view',
  templateUrl: './policy-enrollment-view.component.html'
})
export class PolicyEnrollmentViewComponent implements OnInit {
  ...
  */
  ...
  /**
   * Toggle between table or cards list depending on resolution screen
   */
  public table = true;
  private subscriptions: Subscription[] = [];
  private MOBILE_RESOLUTION = 768;
  constructor(private translate: TranslateService,
    private authService: AuthService,
    private policyAppService: PolicyApplicationService,
    private notification: NotificationService,
    private windowResizeService: WindowResizeService) {
      // Toggle between table or cards depending on resolution screen
      this.subscriptions.push(this.windowResizeService.onResize.subscribe(() => {
        this.table = (window.innerWidth > this.MOBILE_RESOLUTION) ? true : false;
      }));
     }
  ...
  ngOnInit() {
    ...
    this.newFilterPolicies(this.policyApplicationForm.value);
    /// It is neccessary to validate again because user can refresh de page
    this.table = (window.innerWidth > this.MOBILE_RESOLUTION) ? true : false;
  }
  ...
  buildForm() {
    return new FormGroup({
      policyAppSearchType: new FormControl(this.policyAppSearchType[this.policyAppSearchType
        .findIndex(x => x.value === this.SEARCH_TYPE_CASE_ALL)].value),
  ...
  */
  ...
  get labelTableOrCards(): string {
    return this.table ? 'POLICY.APPLICATION.TITLE_CARDS_BUTTON' : 'POLICY.APPLICATION.TITLE_TABLE_BUTTON';
  }

  get toggleTableCards() {
    return this.table = !this.table;
  }

  get getUser(): UserInformationModel {
    return this.user;
  }
}
