...
@Component({
  selector: 'app-load-coverages',
  templateUrl: './load-coverages.component.html'
})
export class LoadCoveragesComponent implements OnInit, OnDestroy {

  /**
   * Subscription save coverages.
   */
  private subCoveragesSave: Subscription;

  /**
   * Subscription submit coverages.
   */
  private subCoveragesSubmit: Subscription;
  ...
  /**
   * Constant for define how many elements splice in coverages array
   */
   ...
   */
  private ERROR_DATA_TITLE = 'COVERAGES.LOAD_COVERAGES.TITLE_01';
  private ERROR_DATA_MSG = 'POLICY.POLICY_COVERAGES.DATA_NOT_FOUND_ERROR_MSG';
   ...
   */
  ngOnDestroy() {
    if (this.subCoveragesSave) { this.subCoveragesSave.unsubscribe(); }
    if (this.subCoveragesSubmit) { this.subCoveragesSubmit.unsubscribe(); }
  }
  ...
  removeDocument(document: FileDocument, e) {
    this.coverages.splice(this.coverages.indexOf(document), this.ERASE_ONE_ELEMENT);
    this.errorsResponse = { errors: [] };
    e.preventDefault();
  }

  /**
   * This function allow remove all coverages.
   */
  async removeAllCoverages(e) {
    e.preventDefault();
  ...
  dragOver(e: DragEvent) {
    ...
    this.dragging = true;
    this.selectButton = this.coverages.length === 0 ? this.DRAG_HERE : this.SELECT_FILE;
  }
  ...
  async loadFiles(files: FileList) {
    let hasError = null;
    if (files.length + this.coverages.length <= this.configuration.networkProvidersMaxFileQuantity) {
      for (let i = 0; i < files.length; i++) {
        const file: File = files[i];
        const AllowedTypes = this.allowedTypes.indexOf(file.name.toLowerCase().split('.').splice(-1)[0]) >= 0;
        const fileNotAlready = this.coverages.filter(p => p.file.name === file.name).length === 0;
  ...
  saveCoverages() {
    if (this.coverages.length > 0) {
      const documentPromise = Observable.fromPromise(
        this.commonService.uploadFirstDocument(this.coverages[0])
      );
      this.subCoveragesSave = documentPromise.subscribe(
        data => {
          this.submitCoverages(data);
        }, error => {
          console.error(error);
        }
      );
   ...
   */
  submitCoverages(response: UploadResponse) {
    if (response) {
      this.errorsResponse = { errors: [] };
      this.subCoveragesSubmit = this.coveragesService.sendContractProcessingCoverages(response).subscribe(
        data => {
  ...
  handleError(error) {
    ...
    if (error.status === Number('400')) {
      ...
      let messageFound = '';
      let messageToShare = '';
      this.translate.get(`COVERAGES.LOAD_COVERAGES.ERROR_MSGS.${error.error.code}`).subscribe(
        result => {
          messageFound = result;
          let i = 0;
          const ingresedParams = error.error.messageParams.length;
          if (ingresedParams !== null) {
            for (i = 0; i < error.error.messageParams.length; i++) {
              const word = '{'.concat(i.toString()).concat('}');
              messageToShare = messageFound.replace(word, error.error.messageParams[i]);
              messageFound = messageToShare;
            }
          }
        }
      );
      ...
      response = this.notification.showDialog(messageTitle, messageFound, false, okBtn);
      if (response) {
        this.coverages = [];
        this.errorsResponse = { errors: [] };
      }
  ...
  handleErrorMessage(code: string, variables: string[]) {
    if (code) {
      let messageFound = '';
      let messageToShared = '';
      this.translate.get(`COVERAGES.LOAD_COVERAGES.ERROR_MSGS.${code}`).subscribe(
        result => {
          messageFound = result;
          let i = 0;
