...
@Component({
  selector: 'app-information-request-step1-inquiry-default',
  templateUrl: './information-request-step1-inquiry-default.component.html'
})
export class InformationRequestStep1InquiryDefaultComponent implements OnInit, OnDestroy {
  ...
  /***
   * Validated if show input member
   */
  ...
  */
  ...
  public inquiryGeneral = true;
  public inquiryRelated = false;
  public inquiryType = true;
  ...
  ngOnInit() {
    ...
    this.selectTypeOfInquiry();
    this.showPolicyNumber();
    this.getSubjects();
    this.checkIfInquiryIsResponseFromOtherInquiry();
    this.registerInquiryTypeValueChanges();
  }

  selectTypeOfInquiry() {
    if (this.wizard && this.wizard.infoRequestForm.untouched) {
      if (this.user.role_id === Rol.PROVIDER.toString()) {
        this.selectedInquiryGeneral();
      } else {
        this.selectedInquiryRelated();
      }
    }
  }

  registerInquiryTypeValueChanges() {
    this.wizard.infoRequestForm.get('type').valueChanges.subscribe(ch => {
      this.showPolicyNumber();
      switch (ch) {
        case 'inquiryGeneral':
          this.setValidatorsNewRequestInformationDefault();
          break;
        case 'inquiryRelated':
          this.checkIfChangeOptions();
          this.setValidatorsNewRequestInformationRelated();
          break;
        default:
          this.setValidatorsNewRequestInformationDefault();
          break;
  ...
  checkIfChangeOptions() {
    if (this.oldInquiry &&
      (!this.wizard.infoRequestForm.get('policyNumber').value ||
        this.wizard.infoRequestForm.get('policyNumber').value === '')) {
      this.wizard.infoRequestForm.get('policyNumber').setValue(+this.oldInquiry.interestedUser.policyId);
    }
  }
  ...
  checkIfInquiryIsResponseFromOtherInquiry() {
    ...
    if (this.oldInquiry) {
      if (this.user.role_id !== Rol.PROVIDER.toString()) {
        ...
        if (this.oldInquiry && this.oldInquiry.interestedUser && this.oldInquiry.interestedUser.policyId) {
  ...
  setValidatorsNewRequestInformationDefault() {
      this.wizard.member = null;
      this.wizard.memberSearch = [];
      this.wizard.infoRequestForm.get('policyNumber').clearValidators();
      this.wizard.infoRequestForm.get('policyNumber').reset();
      this.wizard.infoRequestForm.get(this.SEARCH_TYPE_CTRL).clearValidators();
      this.wizard.infoRequestForm.get(this.SEARCH_TYPE_CTRL).reset();
      this.wizard.infoRequestForm.updateValueAndValidity();
  }

  setValidatorsNewRequestInformationRelated() {
    this.wizard.infoRequestForm.get('policyNumber').setValidators([Validators.required, Validators.pattern(/^\d+$/)]);
    this.wizard.infoRequestForm.get(this.SEARCH_TYPE_CTRL).setValidators([Validators.required]);
    this.wizard.infoRequestForm.updateValueAndValidity();
  }
  ...
  */
  ...
  showPolicyNumber() {
    if ((Number(this.user.role_id) === Rol.AGENT)
      ...
      if (this.wizard.infoRequestForm.get('type').value === 'inquiryRelated') {
  ...
  handleMemberNext() {
    if (Number(this.user.role_id) === Rol.PROVIDER) {
      ...
      if (this.wizard.infoRequestForm.get('type').value === 'inquiryGeneral') {
        return true;
      }
  ...
  selectedInquiryGeneral() {
    this.wizard.infoRequestForm.patchValue({ type: 'inquiryGeneral' });
  }

  selectedInquiryRelated() {
    this.wizard.infoRequestForm.patchValue({ type: 'inquiryRelated' });
  }
